...
cd helm-sendbox/message/bpp/test && ./test-on-discover.sh

# Test on_select response
cd helm-sendbox/message/bpp/test && ./test-on-select.sh
```
...
curl -X POST http://onix-bpp-service:8002/bpp/caller/on_discover \
  -H "Content-Type: application/json" \
  -d @example/on_discover.json
```
